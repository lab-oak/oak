...
/*
  ...
  transform).
    ...
    - expected use-case is for when PaddedInputDimensions == 32 and
  InputDimensions <= 32.
...
template <IndexType InputDimensions, IndexType PaddedInputDimensions,
...
static void affine_transform_non_ssse3(std::int32_t *output,
  ...
  constexpr IndexType NumChunks =
      ceil_to_multiple<IndexType>(InputDimensions, 16) / 16;
  ...
  for (IndexType i = 0; i < OutputDimensions; ++i) {
    ...
    for (IndexType j = 0; j < NumChunks; ++j) {
      ...
      __m128i extendedRowLo =
          _mm_srai_epi16(_mm_unpacklo_epi8(row_j, row_j), 8);
      __m128i extendedRowHi =
          _mm_srai_epi16(_mm_unpackhi_epi8(row_j, row_j), 8);
