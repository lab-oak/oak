...
// scheme.
template <typename IntType>
inline void read_leb_128(std::istream &stream, IntType *out,
                         std::size_t count) {
  ...
  static_assert(std::is_signed_v<IntType>,
                "Not implemented for unsigned types");
  ...
  for (std::size_t i = 0; i < count; ++i) {
    ...
    do {
      if (buf_pos == BUF_SIZE) {
        stream.read(reinterpret_cast<char *>(buf),
...
// scheme.
template <typename IntType>
inline void write_leb_128(std::ostream &stream, const IntType *values,
                          std::size_t count) {
