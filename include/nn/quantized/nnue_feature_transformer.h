...
template <std::size_t BlockSize, typename T, std::size_t N,
...
public:
  ...
  static constexpr auto InversePackusEpi16Order =
      invert_permutation(PackusEpi16Order);
  ...
  bool write_parameters(std::ostream &stream) {
    ...
    write_leb_128<WeightType>(stream, weights,
                              HalfDimensions * InputDimensions);
  ...
  std::int32_t transform(const Position &pos,
                         ...
                         OutputType *output, int bucket) const {
    ...
    const auto &accumulation =
        (accumulatorState.acc<HalfDimensions>()).accumulation;
    ...
    for (IndexType p = 0; p < 2; ++p) {
      ...
      constexpr IndexType NumOutputChunks =
          HalfDimensions / 2 / OutputChunkSize;
      ...
      for (IndexType j = 0; j < NumOutputChunks; ++j) {
        const vec_t sum0a = vec_slli_16(
            vec_max_16(vec_min_16(in0[j * 2 + 0], One), Zero), shift);
        const vec_t sum0b = vec_slli_16(
            vec_max_16(vec_min_16(in0[j * 2 + 1], One), Zero), shift);
      ...
      for (IndexType j = 0; j < HalfDimensions / 2; ++j) {
        ...
        output[offset + j] =
            static_cast<OutputType>(unsigned(sum0 * sum1) / 512);
      }
