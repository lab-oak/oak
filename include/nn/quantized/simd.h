...
#ifdef VECTOR
  using type = vec_t;
  static type add(const type &lhs, const type &rhs) {
    return vec_add_16(lhs, rhs);
  }
  static type sub(const type &lhs, const type &rhs) {
    return vec_sub_16(lhs, rhs);
  }
#else
  using type = BiasType;
...
enum UpdateOperation { Add, Sub };

template <typename VecWrapper, UpdateOperation... ops,
          std::enable_if_t<sizeof...(ops) == 0, bool> = true>
typename VecWrapper::type fused(const typename VecWrapper::type &in) {
  return in;
}

template <typename VecWrapper, UpdateOperation update_op,
          UpdateOperation... ops, typename T, typename... Ts,
          std::enable_if_t<is_all_same_v<typename VecWrapper::type, T, Ts...>,
                           bool> = true,
          std::enable_if_t<sizeof...(ops) == sizeof...(Ts), bool> = true>
typename VecWrapper::type fused(const typename VecWrapper::type &in,
                                const T &operand, const Ts &...operands) {
  switch (update_op) {
  case Add:
...
[[maybe_unused]] static void m512_add_dpbusd_epi32(__m512i &acc, __m512i a,
                                                   __m512i b) {
...
[[maybe_unused]] static void m128_add_dpbusd_epi32(__m128i &acc, __m128i a,
                                                   __m128i b) {
...
#if USE_NEON >= 8
[[maybe_unused]] static void
neon_m128_add_dpbusd_epi32(int32x4_t &acc, int8x16_t a, int8x16_t b) {
...
public:
  static constexpr int NumRegs =
      BestRegisterCount<vec_t, WeightType, TransformedFeatureWidth,
                        NumRegistersSIMD>();
