...
template <float c_ucb> class JointBanditData {
  static auto consteval get_name() {
    ...
    std::array<char, 5 + 3 + trailing_precision> name{"ucbp-"};
  ...
  void select(PRNG &device, Outcome &outcome) const noexcept {
    if (_rows < 2) {
      ...
      for (auto i = 0; i < _rows; ++i) {
        ...
        float e = this->p1_prior[i] * c_ucb * std::sqrt(log_N / p1_visits[i]);
        float a = e + q[i];
        if (a > max) {
          max = a;
          outcome.p1_index = i;
    ...
    if (_cols < 2) {
      ...
      for (auto i = 0; i < _cols; ++i) {
        ...
        float e = this->p2_prior[i] * c_ucb * sqrt(log_N / p2_visits[i]);
        float a = e + q[i];
        if (a > max) {
          max = a;
          outcome.p2_index = i;
