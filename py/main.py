...
def inference(
    ...
    # active word
    output.sides[:size, :, :, 1 : network.active_out_dim + 1] = output.active[:size]
    # pokemon hp/word
...
def loss(input: libtrain.EncodedFrameInput, output: net.OutputBuffers):
    ...
    window = 10
    print(torch.cat([output.value[:window], target[:window]], dim=1))
...
def loss(
    input: libtrain.EncodedFrameInput, output: net.OutputBuffers, print_flag=False
):
    size = min(input.size, output.size)
